...
def patch_file(filepath):
    ...
    # 2. Update getQuinteroProducts carefully
    content = re.sub(
        r'(isActive: true,[ \t]*//[^\n]+)\n([ \t]+isStockTracked: true)',
        r'\1\n\2,\n\2provider: p.provider || "Quintero"'.replace('isStockTracked: true', ''),
        content
    )
    # let's be simpler for Quintero
    ...
    # match patterns like isActive: ..., isStockTracked: ...
    # Be careful not to replace the one we just fixed.
    content = content.replace("isStockTracked: p.isStockTracked || false\n        }));", "isStockTracked: p.isStockTracked || false,\n            provider: p.provider\n        }));")
    content = content.replace("isStockTracked: false\n        }));", "isStockTracked: false,\n            provider: p.provider\n        }));")
    # For getProductsFromCycle
    content = content.replace("isStockTracked: (p as any).isStockTracked || false\n        }));", "isStockTracked: (p as any).isStockTracked || false,\n            provider: p.provider\n        }));")
    # For createInventoryProducts
    content = content.replace("isStockTracked: newProduct.isStockTracked\n                });", "isStockTracked: newProduct.isStockTracked,\n                    provider: newProduct.provider\n                });")
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Patched catalog-actions.ts successfully.")

if __name__ == '__main__':
    patch_file('src/actions/admin/catalog-actions.ts')
